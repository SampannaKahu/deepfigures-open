...
def interp(w, i, channel_dim):
    ...
    upper_l = tf.cast(
        tf_concat(1, [i[:, 0:1],
                      tf.floor(i[:, 1:2]),
                      tf.floor(i[:, 2:3])]), dtype=tf.int32
    )
    upper_r = tf.cast(
        tf_concat(1, [i[:, 0:1],
                      tf.floor(i[:, 1:2]),
                      tf.math.ceil(i[:, 2:3])]), dtype=tf.int32
    )
    lower_l = tf.cast(
        tf_concat(1, [i[:, 0:1],
                      tf.math.ceil(i[:, 1:2]),
    ...
    upper_l_idx = to_idx(upper_l, tf.shape(input=w))
    upper_r_idx = to_idx(upper_r, tf.shape(input=w))
    lower_l_idx = to_idx(lower_l, tf.shape(input=w))
    lower_r_idx = to_idx(lower_r, tf.shape(input=w))
...
def bilinear_select(
    ...
    interp_indices = tf_concat(
        1, [tf.cast(batch_ids, dtype=tf.float32), pred_y_center_clip, pred_x_center_clip]
    )
