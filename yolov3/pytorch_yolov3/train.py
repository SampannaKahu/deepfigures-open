...
def parse_args():
    ...
    parser.add_argument('--cfg', type=str, default='yolov3_default.cfg',
...
def main():
    ...
    dataset = ArxivDataSet(list_of_files=input_files, shuffle_input=True, get_raw_image=False, ignore_pages_with_no_figures=True)
    ...
    for iter_i in range(iter_state, iter_size + 1):
        ...
        if iter_i % 10 == 0:
            ...
            if random_resize:
                ...
                dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=args.n_cpu)
                dataiterator = iter(dataloader)
