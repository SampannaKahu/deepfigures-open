...
sudo -u $USER_NAME mkdir -p "$WORK"/deepfigures-results/arxiv_data_output
sudo -u $USER_NAME mkdir -p "$WORK"/deepfigures-results/arxiv_data_temp
sudo -u $USER_NAME mkdir -p "$WORK"/deepfigures-results/model_checkpoints
sudo -u $USER_NAME mkdir -p "$WORK"/deepfigures-results/weights
...
for i in {0..26}; do
  ...
  if ls "$WORK"/deepfigures-results/pmctable_arxiv_combined* 1>/dev/null 2>&1; then
    ...
    sudo -u $USER_NAME rm -f $WORK/deepfigures-results/weights/*.meta
    [ -f $LATEST_META_FILE ] && sudo -u $USER_NAME cp $LATEST_META_FILE "$WORK"/deepfigures-results/weights/save.ckpt-500000.meta
    ...
    [ -f $LATEST_INDEX_FILE ] && sudo -u $USER_NAME cp $LATEST_INDEX_FILE "$WORK"/deepfigures-results/weights/save.ckpt-500000.index
    ...
    rm -f $WORK/deepfigures-results/weights/*.data*sudo -p $USER_NAME
    [ -f $LATEST_DATA_FILE ] && sudo -u $USER_NAME cp $LATEST_DATA_FILE "$WORK"/deepfigures-results/weights/save.ckpt-500000.data-00000-of-00001

    echo "Moving the figure jsons to the model checkpoint directory of the corresponding model."
    [ -f "$WORK"/deepfigures-results/figure_boundaries.json ] && sudo -u $USER_NAME mv "$WORK"/deepfigures-results/figure_boundaries.json "$LATEST_MODEL_DIR"
    [ -f "$WORK"/deepfigures-results/figure_boundaries_train.json ] && sudo -u $USER_NAME mv "$WORK"/deepfigures-results/figure_boundaries_train.json "$LATEST_MODEL_DIR"
    [ -f "$WORK"/deepfigures-results/figure_boundaries_test.json ] && sudo -u $USER_NAME mv "$WORK"/deepfigures-results/figure_boundaries_test.json "$LATEST_MODEL_DIR"
    ...
    sudo -u $USER_NAME rm -f "$WORK"/deepfigures-results/figure_boundaries.json
    sudo -u $USER_NAME rm -f "$WORK"/deepfigures-results/figure_boundaries_train.json
    sudo -u $USER_NAME rm -f "$WORK"/deepfigures-results/figure_boundaries_test.json
  fi
  ...
  #  ts=$(current_timestamp)sudo -p $USER_NAMEsudo -p $USER_NAME
  ...
  echo "Deleting $WORK/deepfigures-results/arxiv_data_output and $WORK/deepfigures-results/arxiv_data_output."
  sudo -u $USER_NAME mkdir -p /tmp/empty_dir
  rm -rf /tmp/empty_dir/*
  rsync -a --delete /tmp/empty_dir $WORK/deepfigures-results/arxiv_data_output
  rsync -a --delete /tmp/empty_dir $WORK/deepfigures-results/arxiv_data_temp
  ...
  sudo -u $USER_NAME mkdir -p "$WORK"/deepfigures-results/download_cache
  ...
  sudo -u $USER_NAME cp /home/sampanna/df/files_"$i".json "$WORK"/deepfigures-results/files.json

  # Download the files over SSH.
  sudo -u $USER_NAME cat "$WORK"/deepfigures-results/files.json | grep arXiv | awk -F'.' '{print $1}' | awk -F'/' '{print "arxiv_src_"$5".tar"}' >/tmp/rsync.txt
  sudo -u $USER_NAME rsync -Pauv -e 'ssh -i sshkey' --files-from=/tmp/rsync.txt cascades2.arc.vt.edu:/work/cascades/sampanna/deepfigures-results/download_cache/ "$WORK"/deepfigures-results/download_cache
  rm -f /tmp/rsync.txt

  # Generate the data.
  sudo -u $USER_NAME docker run --gpus all -it --volume "$WORK"/deepfigures-results:/work/host-output --volume "$WORK"/deepfigures-results:/work/host-input $CPU_IMAGE python deepfigures/data_generation/arxiv_pipeline.py

  # Prepare the figure_boundaries.json file.
  sudo -u $USER_NAME docker run --gpus all -it --volume "$WORK"/deepfigures-results:/work/host-output --volume "$WORK"/deepfigures-results:/work/host-input $CPU_IMAGE python figure_json_transformer.py
  sudo -u $USER_NAME docker run --gpus all -it --volume "$WORK"/deepfigures-results:/work/host-output --volume "$WORK"/deepfigures-results:/work/host-input $CPU_IMAGE python figure_boundaries_train_test_split.py

  # Trigger the training.
  rm -f "$WORK"/deepfigures-results/train_cid.txt
  CID=$(sudo -u $USER_NAME docker run --cidfile "$WORK"/deepfigures-results/train_cid.txt -d --gpus all -it --volume "$WORK"/deepfigures-results:/work/host-output --volume "$WORK"/deepfigures-results:/work/host-input $GPU_IMAGE python /work/vendor/tensorboxresnet/tensorboxresnet/train.py --hypes /work/host-input/weights/hypes.json --gpu 0 --logdir /work/host-output)
  sudo -u $USER_NAME sleep 12h && sudo -u $USER_NAME docker stop -t 60 "$CID" # Sleep for 12 hours and then invoke the stop command (with a 60 sec timeout)
